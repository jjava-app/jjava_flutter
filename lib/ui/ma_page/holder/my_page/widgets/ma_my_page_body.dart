...
class MaPageBody extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final meAsync = ref.watch(maMyPageVMProvider);

    return meAsync.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, _) => Center(child: Text('오류: $e')),
      data: (MyPageProfile p) {
        final email = p.email.isEmpty ? '연동 이메일 없음' : p.email;
                ...
                children: [
                  Row(
                    children: [
                      ...
                      MText.s14Bold(p.username, color: MColor.kPrimary.normal),
                    ],
                  ),
                  Row(
                    children: [
                      _RankChip(label: "${p.rank} 위"),
                      const SizedBox(width: 8),
                      _RankChip(label: "${p.score}점"),
                    ],
                  ),
                ],
              ),
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          MText.s20Bold(p.username, color: MColor.kLabel.neutral),

                          const SizedBox(height: 12),
                          MText.h5('설정 학습 난이도', color: MColor.kLabel.alternative),
                          const SizedBox(height: 4),
                          MText.s20Bold(p.levelDisplay, color: MColor.kPrimary.normal),
                        ],
                      ),
                    ),
